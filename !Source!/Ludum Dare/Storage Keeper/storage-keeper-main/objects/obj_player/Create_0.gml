...
sound_walk = audio_play_sound(snd_walk_a, 1, true);
...
function update_collision_mask() {
	if (is_sitting) {
		if (instance_exists(holded)) {
	        mask_index = spr_player_collision_sit_hold;
	    }
	    else {
	        mask_index = spr_player_collision_sit;
	    }
	} else {
	    if (instance_exists(holded)) {
	        mask_index = spr_player_collision_hold;
...
function collision_if_standup() {
	var current_mask = mask_index;
	if (instance_exists(holded)) {
	    mask_index = spr_player_collision_hold;
	}
	else {
	...
	var collision = place_meeting(x, y - 0.5, obj_solid);
	mask_index = current_mask;
	return collision;
}
